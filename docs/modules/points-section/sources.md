...
### What are they?
---
Sources are a way to award members with points in a [system](/modules/points/systems) automatically for performing an action. Each source can be customized to fit the servers needs.
...
Each source has a name, attacked system, and type, with further customization based on the type of source chosen.
...
- **Source type** - The way a member gains points through the source. The current types are `Messages sent`, `Voice channel time`, and `People invited`.
	- `Messages sent` will trigger after a number of `Messages required` from the member are sent (in channels Monni can see) with a `Cooldown` before messages start counting towards the requirement again.
...
This limit is 5 sources for a normal server, and 10 for a premium server.