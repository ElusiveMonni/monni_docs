...
**First**.
Create a channel where the reaction roles message will be placed, preferably where no one else will talk, otherwise the message will get lost in a list of other messages.

![reaction-roles1](images/reaction-roles/reaction-roles1.png)

**Second**.
 Go to the [*Monni Dashboard*](https://monni.fyi/dashboard), then go to your server and select the roles module. Next, under *Reaction roles* select "New reaction role".

![reaction-roles2](images/reaction-roles/reaction-roles2.png)

**Third**.
 In the menu that has just popped up, press "Configure Message" to open the message editor and in the top left corner click the brush to clear the template message.

![reaction-roles3](images/reaction-roles/reaction-roles3.png)

**Fourth**.
 Create a message to explain what each role's purpose is to make choosing easier for server members.
...
**Fifth**.
Press return and set which channel you'd like your reaction role message to be sent in. Then select the buttons which will be under your embed. (These assign the roles.)
